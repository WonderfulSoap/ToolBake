...
describe("AuthHelper", () => {
  ...
  it("executeWithAccessToken retries on InvalidAccessToken and eventually throws after max retries", async () => {
    ...
    const helper = new AuthHelper(authRepository, tokenRepository);
    let attempts = 0;
    // The implementation retries on InvalidAccessToken, refreshing the token between attempts.
  ...
  it("executeWithAccessToken throws logout error when refresh token is invalid", async () => {
    ...
    const helper = new AuthHelper(authRepository, tokenRepository);
    await expect(helper.executeWithAccessToken(async () => "ok")).rejects.toThrow("invalid refresh");
    expect(authRepository.updateAccessToken).toHaveBeenCalledTimes(2); // initial + 1 retry
