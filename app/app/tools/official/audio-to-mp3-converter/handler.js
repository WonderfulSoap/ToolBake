...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  if (changedWidgetIds && !shouldRun) {
    revokeObjectUrls();
    return buildIdleResponse(audioFiles.length, rateMode);
  }
  ...
  if (!shouldRun) return buildIdleResponse(audioFiles.length, rateMode);
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    const args = buildFfmpegArgs(inputName, outputName, rateMode, cbrBitrate, abrBitrate, vbrQuality);
...
function resolveVbrQuality(value) {
  ...
  if (!Number.isFinite(quality)) return 2;
...
// Resolve the output MP3 file name, ensuring uniqueness when duplicates exist.
...
// Build ffmpeg arguments for MP3 conversion based on user settings.
function buildFfmpegArgs(inputName, outputName, rateMode, cbrBitrate, abrBitrate, vbrQuality) {
  const args = [
    "-y",
    "-i",
    inputName,
...
function buildIdleResponse(fileCount, rateMode) {
  const label = fileCount ? `Ready to convert ${fileCount} audio file(s) to .mp3 (${rateMode}).` : "Upload audio files to begin.";
  return {
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate MP3 downloads."),
    convertError   : "",
