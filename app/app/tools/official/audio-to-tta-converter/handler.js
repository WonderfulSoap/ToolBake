...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    await runCommandWithProgress(ffmpeg, args, index, audioFiles.length, stageLabel, "Target .tta", callback);
...
// Resolve channel mode selection.
...
// Resolve the output TTA file name, ensuring uniqueness when duplicates exist.
...
// Build ffmpeg arguments for TTA conversion based on user settings.
...
function buildIdleResponse(fileCount) {
  const label = fileCount ? `Ready to convert ${fileCount} audio file(s) to .tta.` : "Upload audio files to begin.";
  return {
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate TTA downloads."),
    convertError   : "",
