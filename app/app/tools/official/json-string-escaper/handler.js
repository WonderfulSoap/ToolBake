...
async function handler(inputWidgets, changedWidgetIds) {
  const inputText = inputWidgets["input-text"] || "";

  // Escape the input text as JSON string value
  const escapedValue = JSON.stringify(inputText);

  // Build the result object { "data": ... }
  const resultJson = { data: JSON.parse(escapedValue) };

  // Format and return as JSON string for textarea display
  const resultText = JSON.stringify(resultJson, null, 2);
