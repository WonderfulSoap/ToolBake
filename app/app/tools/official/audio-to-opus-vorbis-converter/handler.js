...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  const codecType = resolveCodecType(inputWidgets.codecType);
  const opusBitrate = resolveBitrate(inputWidgets.opusBitrate, 96, 24, 320);
  const opusVbrMode = resolveOpusVbrMode(inputWidgets.opusVbrMode);
  const opusApplication = resolveOpusApplication(inputWidgets.opusApplication);
  const vorbisQuality = resolveVorbisQuality(inputWidgets.vorbisQuality);

  if (changedWidgetIds && !shouldRun) {
    revokeObjectUrls();
    return buildIdleResponse(audioFiles.length, codecType);
  }
  ...
  if (!shouldRun) return buildIdleResponse(audioFiles.length, codecType);
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    const outputName = resolveOutputName(file.name, index, usedNames, codecType);
    const stageLabel = `Converting ${index + 1}/${audioFiles.length}`;
    const args = buildFfmpegArgs(inputName, outputName, codecType, opusBitrate, opusVbrMode, opusApplication, vorbisQuality);

    await ffmpeg.writeFile(inputName, new Uint8Array(await file.arrayBuffer()));
    await runCommandWithProgress(ffmpeg, args, index, audioFiles.length, stageLabel, `Codec ${codecType}`, callback);
    const outputBuffer = await ffmpeg.readFile(outputName);
    await ffmpeg.deleteFile(inputName);
    await ffmpeg.deleteFile(outputName);

    outputs.push(buildDownloadEntry(outputName, outputBuffer, codecType));
  }
...
// Resolve the selected codec type.
function resolveCodecType(value) {
  const key = String(value || "opus").toLowerCase();
  if (key === "vorbis") return "vorbis";
  return "opus";
}

// Resolve a bitrate input in kbps.
function resolveBitrate(value, fallback, min, max) {
  const bitrate = Number(value);
  if (!Number.isFinite(bitrate)) return fallback;
  return Math.min(max, Math.max(min, Math.round(bitrate)));
}

// Resolve Opus VBR mode selection.
function resolveOpusVbrMode(value) {
  const key = String(value || "on").toLowerCase();
  if (key === "off") return "off";
  if (key === "constrained") return "constrained";
  return "on";
}

// Resolve Opus application selection.
function resolveOpusApplication(value) {
  const key = String(value || "audio").toLowerCase();
  if (key === "voip") return "voip";
  if (key === "lowdelay") return "lowdelay";
  return "audio";
}

// Resolve a Vorbis quality value.
function resolveVorbisQuality(value) {
  const quality = Number(value);
  if (!Number.isFinite(quality)) return 5;
  return Math.min(10, Math.max(0, Math.round(quality)));
}
...
// Resolve the output file name, ensuring uniqueness when duplicates exist.
function resolveOutputName(name, index, usedNames, codecType) {
  const parts = splitFileName(sanitizeFileName(name, `audio-${index + 1}`));
  const base = parts.baseName || `audio-${index + 1}`;
  const ext = codecType === "opus" ? "opus" : "ogg";
...
// Build ffmpeg arguments based on codec selection and user settings.
function buildFfmpegArgs(inputName, outputName, codecType, opusBitrate, opusVbrMode, opusApplication, vorbisQuality) {
  const args = [
    "-y",
    "-i",
    inputName,
  ...
  if (codecType === "opus") {
    args.push("-c:a", "libopus", "-b:a", `${opusBitrate}k`, "-vbr", opusVbrMode, "-application", opusApplication, "-f", "ogg");
  } else {
    args.push("-c:a", "libvorbis", "-q:a", String(vorbisQuality));
  }
...
function buildIdleResponse(fileCount, codecType) {
  const extLabel = codecType === "opus" ? ".opus" : ".ogg";
  const label = fileCount ? `Ready to convert ${fileCount} audio file(s) to ${extLabel}.` : "Upload audio files to begin.";
  return {
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate Opus/Vorbis downloads."),
    convertError   : "",
...
function buildDownloadEntry(fileName, buffer, codecType) {
  return {
    fileName,
    buffer,
    mime: codecType === "opus" ? "audio/opus" : "audio/ogg",
  };
}
