...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  const mode = resolveEncodingMode(inputWidgets.qualityMode);
  const quality = resolveVorbisQuality(inputWidgets.vorbisQuality);
  const bitrate = resolveBitrate(inputWidgets.targetBitrate, 192);

  if (changedWidgetIds && !shouldRun) {
    revokeObjectUrls();
    return buildIdleResponse(audioFiles.length, mode);
  }
  ...
  if (!shouldRun) return buildIdleResponse(audioFiles.length, mode);
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    const args = buildFfmpegArgs(inputName, outputName, mode, quality, bitrate);

    await ffmpeg.writeFile(inputName, new Uint8Array(await file.arrayBuffer()));
    await runCommandWithProgress(ffmpeg, args, index, audioFiles.length, stageLabel, "Target .ogg", callback);
...
// Resolve the selected encoding mode for Vorbis.
function resolveEncodingMode(value) {
  const key = String(value || "quality").toLowerCase();
  if (key === "bitrate") return "bitrate";
  return "quality";
}
...
function resolveVorbisQuality(value) {
  ...
  if (!Number.isFinite(quality)) return 6;
...
function resolveBitrate(value, fallback) {
  ...
  return Math.min(500, Math.max(64, Math.round(bitrate)));
}
...
// Resolve the output OGG file name, ensuring uniqueness when duplicates exist.
...
// Build ffmpeg arguments for OGG (Vorbis) conversion based on user settings.
function buildFfmpegArgs(inputName, outputName, mode, quality, bitrate) {
  const args = [
    "-y",
    "-i",
    inputName,
  ...
  if (mode === "quality") args.push("-q:a", String(quality));
  if (mode === "bitrate") args.push("-q:a", String(mapBitrateToQuality(bitrate)));
  args.push(outputName);
  return args;
}

// Build an idle response when inputs change but conversion is not requested yet.
function buildIdleResponse(fileCount, mode) {
  const label = fileCount ? `Ready to convert ${fileCount} audio file(s) to .ogg (${mode}).` : "Upload audio files to begin.";
  return {
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate OGG downloads."),
    convertError   : "",
...
// Map a target bitrate to a libvorbis quality value for stable encoding.
function mapBitrateToQuality(bitrate) {
  const value = Number(bitrate);
  if (!Number.isFinite(value)) return 5;
  if (value >= 320) return 8;
  if (value >= 256) return 7;
  if (value >= 192) return 6;
