...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  const compressionLevel = resolveCompressionLevel(inputWidgets.compressionLevel);

  if (changedWidgetIds && !shouldRun) {
    revokeObjectUrls();
    return buildIdleResponse(audioFiles.length);
  }
  ...
  if (!shouldRun) return buildIdleResponse(audioFiles.length);
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    const args = buildFfmpegArgs(inputName, outputName, compressionLevel);

    await ffmpeg.writeFile(inputName, new Uint8Array(await file.arrayBuffer()));
    await runCommandWithProgress(ffmpeg, args, index, audioFiles.length, stageLabel, "Target .flac", callback);
...
// Resolve the requested FLAC compression level.
function resolveCompressionLevel(value) {
  const level = Number(value);
  if (!Number.isFinite(level)) return 5;
  return Math.min(12, Math.max(0, Math.round(level)));
}
...
// Resolve the output FLAC file name, ensuring uniqueness when duplicates exist.
...
// Build ffmpeg arguments for FLAC conversion based on user settings.
function buildFfmpegArgs(inputName, outputName, compressionLevel) {
  const args = [
    "-y",
    "-i",
    inputName,
...
function buildIdleResponse(fileCount) {
  const label = fileCount ? `Ready to convert ${fileCount} audio file(s) to .flac.` : "Upload audio files to begin.";
  return {
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate FLAC downloads."),
    convertError   : "",
