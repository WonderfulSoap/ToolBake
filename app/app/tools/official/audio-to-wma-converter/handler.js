...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  const profile = resolveWmaProfile(inputWidgets.wmaProfile);
  const bitrate = resolveBitrate(inputWidgets.targetBitrate, 192);

  if (changedWidgetIds && !shouldRun) {
    revokeObjectUrls();
    return buildIdleResponse(audioFiles.length, profile);
  }
  ...
  if (!shouldRun) return buildIdleResponse(audioFiles.length, profile);
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    const args = buildFfmpegArgs(inputName, outputName, profile, bitrate);

    await ffmpeg.writeFile(inputName, new Uint8Array(await file.arrayBuffer()));
    await runCommandWithProgress(ffmpeg, args, index, audioFiles.length, stageLabel, "Target .wma", callback);
...
// Resolve the selected WMA profile.
function resolveWmaProfile(value) {
  const key = String(value || "standard").toLowerCase();
  return key === "standard" ? "standard" : "standard";
}
...
function resolveBitrate(value, fallback) {
  ...
  return Math.min(320, Math.max(64, Math.round(bitrate)));
}
...
// Resolve the output WMA file name, ensuring uniqueness when duplicates exist.
...
// Build ffmpeg arguments for WMA conversion based on user settings.
function buildFfmpegArgs(inputName, outputName, profile, bitrate) {
  const codec = "wmav2";
  const args = [
    "-y",
    "-i",
  ...
  if (profile !== "pro") args.push("-ar", "48000");
  args.push(outputName);
  return args;
}

// Build an idle response when inputs change but conversion is not requested yet.
function buildIdleResponse(fileCount, profile) {
  const label = fileCount ? `Ready to convert ${fileCount} audio file(s) to .wma (${profile}).` : "Upload audio files to begin.";
  return {
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate WMA downloads."),
    convertError   : "",
