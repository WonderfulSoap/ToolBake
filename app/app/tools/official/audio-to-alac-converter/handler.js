...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  const container = resolveContainer(inputWidgets.outputContainer);

  if (changedWidgetIds && !shouldRun) {
    revokeObjectUrls();
    return buildIdleResponse(audioFiles.length, container);
  }
  ...
  if (!shouldRun) return buildIdleResponse(audioFiles.length, container);
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    const outputName = resolveOutputName(file.name, index, usedNames, container);
    const stageLabel = `Converting ${index + 1}/${audioFiles.length}`;
    const args = buildFfmpegArgs(inputName, outputName, container);

    await ffmpeg.writeFile(inputName, new Uint8Array(await file.arrayBuffer()));
    await runCommandWithProgress(ffmpeg, args, index, audioFiles.length, stageLabel, `Target ${container}`, callback);
    const outputBuffer = await ffmpeg.readFile(outputName);
    await ffmpeg.deleteFile(inputName);
    await ffmpeg.deleteFile(outputName);

    outputs.push(buildDownloadEntry(outputName, outputBuffer, container));
  }
...
// Resolve output container selection.
function resolveContainer(value) {
  const key = String(value || "m4a").toLowerCase();
  if (key === "caf") return "caf";
  return "m4a";
}

// Resolve requested bit depth for ALAC.
...
// Resolve the output ALAC file name, ensuring uniqueness when duplicates exist.
function resolveOutputName(name, index, usedNames, container) {
  ...
  const ext = container === "caf" ? "caf" : "m4a";
...
// Build ffmpeg arguments for ALAC conversion based on user settings.
function buildFfmpegArgs(inputName, outputName, container) {
  const args = [
    "-y",
    "-i",
    inputName,
  ...
  if (container === "caf") args.push("-f", "caf");
  args.push(outputName);
  return args;
}

// Build an idle response when inputs change but conversion is not requested yet.
function buildIdleResponse(fileCount, container) {
  const extLabel = container === "caf" ? ".caf" : ".m4a";
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate ALAC downloads."),
    convertError   : "",
...
function buildDownloadEntry(fileName, buffer, container) {
  return {
    fileName,
    buffer,
    mime: container === "caf" ? "audio/x-caf" : "audio/mp4",
  };
}
