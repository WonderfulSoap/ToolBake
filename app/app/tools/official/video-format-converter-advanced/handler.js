...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  const extraArgs = normalizeExtraArgs(inputWidgets.extraArgs);

  if (changedWidgetIds && changedWidgetIds !== "convertTrigger") {
    revokeObjectUrls();
    return buildIdleResponse(videoFiles.length, targetFormat, remuxOnly, extraArgs, videoFiles);
  }
  ...
  if (!targetFormat) {
    revokeObjectUrls();
    return buildFormatMissingResponse(videoFiles);
  }

  if (changedWidgetIds !== "convertTrigger") {
    return buildIdleResponse(videoFiles.length, targetFormat, remuxOnly, extraArgs, videoFiles);
  }
  ...
  try {
    ...
    for (let index = 0; index < videoFiles.length; index += 1) {
      ...
      const args = buildFfmpegArgs(inputName, outputName, remuxOnly, extraArgs);
...
// Normalize custom ffmpeg arguments from a textarea payload.
function normalizeExtraArgs(value) {
  if (typeof value !== "string") return [];
  const trimmed = value.trim();
  if (!trimmed) return [];
  return parseArgsText(trimmed);
}
...
// Build ffmpeg args with optional remux and extra arguments.
function buildFfmpegArgs(inputName, outputName, remuxOnly, extraArgs) {
...
function buildIdleResponse(fileCount, format, remuxOnly, extraArgs, files) {
  ...
  return {
    ...
    commandPreview : buildCommandPreviewHtml(files, format, remuxOnly, extraArgs),
  };
}
...
function buildEmptyResponse() {
  return {
    ...
    commandPreview : buildPlaceholderHtml("Upload videos to preview the ffmpeg command."),
  };
}

// Build a response when the target format is missing.
function buildFormatMissingResponse(files) {
  return {
    ...
    commandPreview : buildCommandPreviewHtml(files, "", false, []),
  };
}
...
// Build the command preview HTML for all files.
function buildCommandPreviewHtml(files, format, remuxOnly, extraArgs) {
  if (!files.length) return buildPlaceholderHtml("Upload videos to preview the ffmpeg command.");
  if (!format) return buildPlaceholderHtml("Enter a target format to preview the command.");
  const entries = files.map((file, index) => {
    const inputName = ensureInputName(file.name, index);
    const outputName = resolveOutputName(inputName, file.name, format, index);
    const args = buildFfmpegArgs(inputName, outputName, remuxOnly, extraArgs);
    const command = ["ffmpeg", ...args].map((arg) => quoteArg(arg)).join(" ");
    return `
      <div class="rounded-sm border border-border/60 bg-muted/40 px-3 py-2">
        <div class="text-[11px] uppercase tracking-wide text-muted-foreground">Video ${index + 1}</div>
        <div class="mt-1 font-mono text-[12px] text-foreground break-all">${escapeHtml(command)}</div>
      </div>
    `;
  });
  return `<div class="space-y-2">${entries.join("")}</div>`;
}

// Quote args for a readable command preview.
function quoteArg(value) {
  const text = String(value || "");
  if (!text) return "\"\"";
  if (/[^\w@%+=:,./-]/.test(text)) return `"${text.replace(/"/g, "\\\"")}"`;
  return text;
}
...
// Parse multiline extra args into tokens, respecting simple quotes.
function parseArgsText(text) {
  ...
  for (const line of lines) {
    ...
    if (!trimmed) continue;
    tokens.push(...tokenizeArgs(trimmed));
  }
  return tokens;
}

// Tokenize a single line of arguments with basic quote support.
