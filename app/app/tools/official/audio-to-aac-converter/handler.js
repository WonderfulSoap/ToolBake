...
async function handler(inputWidgets, changedWidgetIds, callback) {
  ...
  const rateMode = resolveRateMode(inputWidgets.rateMode);
  const outputContainer = resolveOutputContainer(inputWidgets.outputContainer);
  const cbrBitrate = resolveBitrate(inputWidgets.cbrBitrate, 192);
  const abrBitrate = resolveBitrate(inputWidgets.abrBitrate, 192);
  const vbrQuality = resolveVbrQuality(inputWidgets.vbrQuality);
  const aacProfile = resolveAacProfile(inputWidgets.aacProfile);

  if (changedWidgetIds && !shouldRun) {
    revokeObjectUrls();
    return buildIdleResponse(audioFiles.length, rateMode, outputContainer);
  }
  ...
  if (!shouldRun) return buildIdleResponse(audioFiles.length, rateMode, outputContainer);
  ...
  for (let index = 0; index < audioFiles.length; index += 1) {
    ...
    const outputName = resolveOutputName(file.name, index, usedNames, outputContainer);
    const stageLabel = `Converting ${index + 1}/${audioFiles.length}`;
    const args = buildFfmpegArgs(inputName, outputName, rateMode, cbrBitrate, abrBitrate, vbrQuality, aacProfile, outputContainer);

    await ffmpeg.writeFile(inputName, new Uint8Array(await file.arrayBuffer()));
    await runCommandWithProgress(ffmpeg, args, index, audioFiles.length, stageLabel, `Mode ${rateMode}`, callback);
    const outputBuffer = await ffmpeg.readFile(outputName);
    await ffmpeg.deleteFile(inputName);
    await ffmpeg.deleteFile(outputName);

    outputs.push(buildDownloadEntry(outputName, outputBuffer, outputContainer));
  }
...
// Resolve bitrate mode selection.
function resolveRateMode(value) {
  const key = String(value || "CBR").toUpperCase();
  if (key === "VBR" || key === "ABR") return key;
  return "CBR";
}

// Resolve the output container selection.
function resolveOutputContainer(value) {
  const key = String(value || "m4a").toLowerCase();
  if (key === "aac") return "aac";
  return "m4a";
}

// Resolve a bitrate input in kbps.
function resolveBitrate(value, fallback) {
  const bitrate = Number(value);
  if (!Number.isFinite(bitrate)) return fallback;
  return Math.min(320, Math.max(32, Math.round(bitrate)));
}

// Resolve a VBR quality value.
function resolveVbrQuality(value) {
  const quality = Number(value);
  if (!Number.isFinite(quality)) return 3;
  return Math.min(9, Math.max(0, Math.round(quality)));
}

// Resolve AAC profile selection.
function resolveAacProfile(value) {
  const key = String(value || "auto").toLowerCase();
  if (key === "lc") return "aac_low";
  if (key === "he") return "aac_he";
  if (key === "hev2") return "aac_he_v2";
  return null;
}
...
// Resolve the output AAC file name, ensuring uniqueness when duplicates exist.
function resolveOutputName(name, index, usedNames, container) {
  const parts = splitFileName(sanitizeFileName(name, `audio-${index + 1}`));
  const base = parts.baseName || `audio-${index + 1}`;
  const ext = container === "aac" ? "aac" : "m4a";
...
// Build ffmpeg arguments for AAC conversion based on user settings.
function buildFfmpegArgs(inputName, outputName, rateMode, cbrBitrate, abrBitrate, vbrQuality, profile, container) {
  const args = ["-y", "-i", inputName];
  if (container === "aac") {
    args.push("-map", "0:a", "-vn");
  } else {
    args.push("-map", "0", "-c:v", "copy");
  }
  args.push("-map_metadata", "0", "-map_chapters", "0", "-c:a", "aac");
  if (container === "aac") args.push("-f", "adts");
  if (profile) args.push("-profile:a", profile);
  if (rateMode === "VBR") {
    args.push("-q:a", String(vbrQuality));
  } else if (rateMode === "ABR") {
    args.push("-b:a", `${abrBitrate}k`, "-abr", "1");
  } else {
    args.push("-b:a", `${cbrBitrate}k`);
  }
...
function buildIdleResponse(fileCount, rateMode, container) {
  const extLabel = container === "aac" ? ".aac" : ".m4a";
  const label = fileCount ? `Ready to convert ${fileCount} audio file(s) to ${extLabel} (${rateMode}).` : "Upload audio files to begin.";
  return {
...
function buildEmptyResponse() {
  return {
    convertOutput  : buildPlaceholderHtml("Upload audio files to generate AAC downloads."),
    convertError   : "",
...
function buildDownloadEntry(fileName, buffer, container) {
  return {
    fileName,
    buffer,
    mime: container === "aac" ? "audio/aac" : "audio/mp4",
  };
}
