...
func (v *AccessTokenHeaderValidator) ValidateAccessTokenHeader(ctx *gin.Context) (entity.UserEntity, error) {
	...
	if err != nil {
		...
		return entity.UserEntity{}, error_code.NewErrorWithErrorCode(error_code.InternalServerError, "Unexpected validate user error")
	}
	if !ok {
		logger.Errorf(ctx, "access token is not valid")
		return entity.UserEntity{}, error_code.NewErrorWithErrorCode(error_code.InvalidAccessToken, "Access token is invalid")
	}
	...
	if err != nil {
		...
		return entity.UserEntity{}, error_code.NewErrorWithErrorCode(error_code.InternalServerError, "Unexpected get user error")
	}
	if !exists {
		logger.Errorf(ctx, "user not found: %s", userID)
		return entity.UserEntity{}, error_code.NewErrorWithErrorCode(error_code.UserNotFound, "User not found")
	}
